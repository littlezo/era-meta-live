# Douyu Platform Package

Douyu 平台包是统一直播平台接入层的 Douyu 平台实现，提供了 Douyu 直播平台的各项功能接入。

## 功能实现

### 1. 直播列表获取

实现了 Douyu 平台的直播列表获取功能，支持按分类和页码获取直播列表。

### 2. 直播间信息获取

实现了 Douyu 平台的直播间信息获取功能，包括直播间标题、主播信息、直播状态等。

### 3. 直播流 URL 获取

实现了 Douyu 平台的直播流 URL 获取功能，支持多种画质选择。

### 4. 主播信息获取

实现了 Douyu 平台的主播信息获取功能，包括主播名称、头像、直播状态等。

### 5. 消息监听

实现了 Douyu 平台的消息监听功能，支持监听直播间的弹幕、礼物、超级弹幕等消息。

### 6. 认证相关功能

实现了 Douyu 平台的认证相关功能，包括获取认证 URL、登录、刷新令牌等。
...
Douyu 平台使用 shared 包中的日志系统，支持配置日志级别、输出目标等。
...
let platform = DouyuPlatform::new(config).unwrap();
```
