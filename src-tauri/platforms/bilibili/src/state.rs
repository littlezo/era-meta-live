...
fn logger() -> &'static Logger {
    LOGGER.get_or_init(|| {
        Logger::new(Some(PlatformType::Bilibili), "bilibili::state")
    })
}
...
impl BilibiliState {
    ...
    pub async fn generate_w_webid(&self) -> Result<String, String> {
        ...
        logger().debug(format!("Generating w_webid: GET {}", url));
        logger().debug(format!(
        ...
        logger().debug(format!("w_webid extracted: {}", w_webid));
        {
            let mut guard = self.w_webid.lock().unwrap();
