...
#[tokio::test]
async fn test_douyin_platform_creation() {
    // 初始化日志系统
    init_logger().unwrap();
    
    // 测试创建 DouyinPlatform 实例
    let config = PlatformConfig::default();
    let platform = DouyinPlatform::new(config);
    assert!(platform.is_ok());
    
    // 测试平台类型
    let platform = platform.unwrap();
    assert_eq!(platform.platform_type(), PlatformType::Douyin);
}

#[tokio::test]
async fn test_douyin_fetch_live_list() {
    // 初始化日志系统
    init_logger().unwrap();
...
#[tokio::test]
async fn test_douyin_fetch_room_info() {
    // 初始化日志系统
    init_logger().unwrap();
