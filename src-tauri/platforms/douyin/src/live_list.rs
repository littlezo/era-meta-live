...
use crate::web_api::DEFAULT_USER_AGENT;
...
pub async fn fetch_live_list(
...
) -> Result<(DouyinLiveListResponse, Option<Value>), String> {
    ...
    // 先获取结构化数据
    let api_response = local_client
        .get_json_with_headers::<DouyinPartitionApiResponse>(&url, Some(headers))
        .await
        .map_err(|e| format!("Network error fetching Douyin room list: {}", e))?;
    
    // 转换为原始JSON数据，用于返回
    let raw_json = serde_json::to_value(&api_response)
        .map_err(|e| format!("Failed to convert to raw JSON: {}", e))?;
