...
#[tokio::test]
async fn test_huya_platform_creation() {
    // 初始化日志系统
    init_logger().unwrap();
    
    // 测试创建 HuyaPlatform 实例
    let config = PlatformConfig::default();
    let platform = HuyaPlatform::new(config);
    assert!(platform.is_ok());
    
    // 测试平台类型
    let platform = platform.unwrap();
    assert_eq!(platform.platform_type(), PlatformType::Huya);
}

#[tokio::test]
async fn test_huya_fetch_live_list() {
    // 初始化日志系统
    init_logger().unwrap();
...
#[tokio::test]
async fn test_huya_fetch_room_info() {
    // 初始化日志系统
    init_logger().unwrap();
