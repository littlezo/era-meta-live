...
export interface Message {
  ...
  content: string;     // The actual message text or system message content
  badgeName?: string;
  badgeLevel?: string;  // String for display (e.g., fan badge level)
  color?: string;       // For UI customization of message text
  room_id?: string;     // The room ID this message belongs to (useful for multi-room contexts or debugging)
  // Add any other fields that are common across platforms for display in MessageList
} 

export interface MessageOverlayInstance {
  sendComment?: (comment: {
    id?: string;
    txt: string;
    duration?: number;
...
export interface MessageRenderOptions {
  ...
  buildCommentOptions?: (message: Message) => {
    duration?: number;
    mode?: string;
    style?: Record<string, string>;
  };
