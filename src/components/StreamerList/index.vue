...
import { useLiveRooms } from './composables/useLiveRooms.ts'
...
const categoryId = computed(() => {
  if (!props.selectedCategory?.cate2Href) return null;
  
  // 提取分类ID，兼容不同平台的分类数据结构
  const href = props.selectedCategory.cate2Href;
  
  // 对于不同平台，分类ID的提取方式可能不同
  // 这里使用统一的方式处理，具体实现由后端完成
  if (props.platform === 'douyin') {
    // 抖音分类格式: 例如 "partition_1_233"
    const parts = href.split('_');
    return parts.length >= 1 ? parts[parts.length - 1] : null;
  } else if (props.platform === 'douyu') {
      // 斗鱼分类格式: 直接返回ID
      // 由于CategorySelectedEvent没有cate2Id属性，这里使用统一的方式处理
      return null;
    } else {
    // 其他平台，尝试从categoriesData中查找
    if (Array.isArray(props.categoriesData)) {
      for (const c1 of props.categoriesData) {
...
const { 
  ...
  loadInitialRooms, 
  loadMoreRooms 
} = useLiveRooms(props.platform, categoryId);
